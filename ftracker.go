...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	meanSpeedMetrsInSec := meanSpeed(action, duration) * kmhInMsec
	quatro := math.Pow(meanSpeedMetrsInSec, 2)
	heightInMetrs := height / cmInM
	return ((walkingCaloriesWeightMultiplier*weight + (quatro/heightInMetrs)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
